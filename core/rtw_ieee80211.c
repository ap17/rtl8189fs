...
#define DEFAULT_RANDOM_MACADDR 1
	...
	/* Prepare default random mac addr */
#if DEFAULT_RANDOM_MACADDR
	*((u32 *)(&def_mac[2])) = rtw_random32();
	def_mac[0] = 0x00;
	def_mac[1] = 0xe0;
	def_mac[2] = 0x4c;
#else
	...
	/* Try the user-specified mac address */
	if (rtw_initmac) {								// if the user specified the mac address
		int jj,kk;
		u8 ll;
		for (jj = 0, kk = 0; jj < ETH_ALEN; jj++, kk += 3) {
			ll=key_char2num(rtw_initmac[kk]);
			if(ll == 0xff) goto err1;
			mac[jj] = ll << 4;
			ll=key_char2num(rtw_initmac[kk+1]);
			if(ll == 0xff) goto err1;
			mac[jj] |= ll;
		}

		if (rtw_check_invalid_mac_address(mac, _TRUE) == _FALSE) {		// if the user specified mac address is CORRECT ...
			printk("rtl8189fs: Use the user-specified mac addr: "MAC_FMT"\n", MAC_ARG(mac));
			_rtw_memcpy(out, mac, ETH_ALEN);
			goto done;
		}

err1:		printk("rtl8189fs: User-specified mac addr is incorrect.\n");
	}

	/* Try to read mac addr from DT */
	if (np &&
	   (addr = of_get_property(np, "local-mac-address", &len)) &&
	    len == ETH_ALEN) {								// if mac addr IS SPECIFIED in DT ...
		memcpy(mac, addr, ETH_ALEN);						// copy data because of dif. types of "mac" and "addr"
		if (rtw_check_invalid_mac_address(mac, _TRUE) == _TRUE) {		// if mac addr is INCORRECT ...
			printk(__FILE__ ":%s: mac addr from DT is incorrect : "MAC_FMT"\n",  __func__, MAC_ARG(mac));
			goto efuse;
		} else {								// mac addr is CORRECT
			printk(__FILE__ ":%s: Use mac addr. from DT : "MAC_FMT"\n", __func__, MAC_ARG(mac));
			_rtw_memcpy(out, mac, ETH_ALEN);
			goto done;
		}
	} else
		printk(__FILE__ ":%s: No mac addr. is specified in DT.\n", __func__);
...
#if 0									// can't use Efuse mac addr in case of 8189FS
	if (hw_mac_addr) {
		_rtw_memcpy(mac, hw_mac_addr, ETH_ALEN);
		printk(__FILE__ ":%s: Efuse mac addr = "MAC_FMT"\n", __func__, MAC_ARG(mac));
		goto err_chk;
	}
#else
	printk(__FILE__ ":%s: It is not possible to define rtl8189fs mac addr from Efuse.\n", __func__);
#endif // 0

	printk(__FILE__ ":%s: Use default mac addr.: "MAC_FMT"\n",  __func__, MAC_ARG(def_mac));
	_rtw_memcpy(out, def_mac, ETH_ALEN);

done:
	DBG_871X("%s mac addr:"MAC_FMT"\n", __func__, MAC_ARG(out));
//	printk(__FILE__ ":%s: mac addr = "MAC_FMT"\n", __func__, MAC_ARG(out));
}
