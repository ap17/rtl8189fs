 ...
 * 4.15 def:
 *      struct cfg80211_wifidirect_info *pcfg80211_wdinfo = &padapter->cfg80211_wdinfo;
 *      pcfg80211_wdinfo->padapter = padapter;
 *      timer_setup( &pcfg80211_wdinfo->remain_on_ch_timer, ro_ch_timer_process, 0 );
 */
static void ro_ch_timer_process(struct timer_list *t)                                                                   // --- AP ---
{
	struct cfg80211_wifidirect_info *pcfg80211_wdinfo = from_timer(pcfg80211_wdinfo, t, remain_on_ch_timer);        // --- AP ---
	_adapter *adapter = pcfg80211_wdinfo->padapter;                                                                 // --- AP ---
#endif // 4.15
//---------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
	...
	_init_timer( &pcfg80211_wdinfo->remain_on_ch_timer, padapter->pnetdev, ro_ch_timer_process, padapter );
#else // 4.15
	pcfg80211_wdinfo->padapter = padapter;                                                          // --- AP ---
	timer_setup( &pcfg80211_wdinfo->remain_on_ch_timer, ro_ch_timer_process, 0 );                   // --- AP ---
#endif // 4.15
//------------------------------------------------------------------------------
}
 ...
 */
static void reset_ch_sitesurvey_timer_process (struct timer_list *t)                                    // --- AP ---
{
	struct wifidirect_info *pwdinfo = from_timer(pwdinfo, t, reset_ch_sitesurvey);                  // --- AP ---
	_adapter *adapter = pwdinfo->padapter;                                                          // --- AP ---
 #endif // 4.15
//-------------------------------------------------------------------------
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,15,0))
static void reset_ch_sitesurvey_timer_process1 (void *FunctionContext)
{
	_adapter *adapter = (_adapter *)FunctionContext;
	struct  wifidirect_info         *pwdinfo = &adapter->wdinfo;
 ...
 */
static void reset_ch_sitesurvey_timer_process2 (struct timer_list *t)                                   // --- AP ---
{
	struct wifidirect_info *pwdinfo = from_timer(pwdinfo, t, reset_ch_sitesurvey2);                 // --- AP ---
	_adapter *adapter = pwdinfo->padapter;                                                          // --- AP ---
#endif // 4.15
//-------------------------------------------------------------------------
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,15,0))
static void reset_ch_sitesurvey_timer_process3 (void *FunctionContext)                          // --- AP ---
{
	_adapter *adapter = (_adapter *)FunctionContext;
	struct  wifidirect_info         *pwdinfo = &adapter->wdinfo;
 ...
 */
static void restore_p2p_state_timer_process (struct timer_list *t)                                      // --- AP ---
{
	struct wifidirect_info *pwdinfo = from_timer(pwdinfo, t, restore_p2p_state_timer);              // --- AP ---
	_adapter *adapter = pwdinfo->padapter;                                                          // --- AP ---
#endif // 4.15
//-------------------------------------------------------------------------
 ...
 */
static void pre_tx_scan_timer_process (struct timer_list *t)                                            // --- AP ---
{
	struct wifidirect_info *pwdinfo = from_timer(pwdinfo, t, pre_tx_scan_timer);                    // --- AP ---
	_adapter *adapter = pwdinfo->padapter;                                                          // --- AP ---
#endif // 4.15
//-------------------------------------------------------------------------
 ...
 */
static void find_phase_timer_process (struct timer_list *t)                                             // --- AP ---
{
	struct wifidirect_info *pwdinfo = from_timer(pwdinfo, t, find_phase_timer);                     // --- AP ---
	_adapter *adapter = pwdinfo->padapter;                                                          // --- AP ---
#endif // 4.15
//-------------------------------------------------------------------------
...
//------------------------------------------------------------------------
	...
	_init_timer( &pwdinfo->reset_ch_sitesurvey2, padapter->pnetdev, reset_ch_sitesurvey_timer_process2, padapter );
#else // 4.15
	timer_setup( &pwdinfo->find_phase_timer, find_phase_timer_process, 0 );                                         // --- AP ---
	timer_setup( &pwdinfo->restore_p2p_state_timer, restore_p2p_state_timer_process, 0 );                           // --- AP ---
	timer_setup( &pwdinfo->pre_tx_scan_timer, pre_tx_scan_timer_process, 0 );                                       // --- AP ---
	timer_setup( &pwdinfo->reset_ch_sitesurvey, reset_ch_sitesurvey_timer_process, 0 );                             // --- AP ---
	timer_setup( &pwdinfo->reset_ch_sitesurvey2, reset_ch_sitesurvey_timer_process2, 0 );                           // --- AP ---
#endif // 4.15
//------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------
			...
			reset_ch_sitesurvey_timer_process2( padapter );
#else // 4.15
			reset_ch_sitesurvey_timer_process1( padapter );                                 // --- AP ---
			reset_ch_sitesurvey_timer_process3( padapter );                                 // --- AP ---
#endif // 4.15
//---------------------------------------------------------------------------------
