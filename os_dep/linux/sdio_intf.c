...
_adapter *rtw_sdio_if1_init(struct dvobj_priv *dvobj)
{
	...
	PSDIO_DATA psdio = &dvobj->intf_data;					// --- AP ---
...
//	rtw_macaddr_cfg(padapter->mac_addr,  get_hal_mac_addr(padapter));				// --- AP ---
//	rtw_macaddr_cfg(&psdio->func->dev, adapter_mac_addr(padapter), get_hal_mac_addr(padapter));	// --- AP ---
	rtw_macaddr_cfg(&psdio->func->dev, padapter->mac_addr, get_hal_mac_addr(padapter));		// --- AP ---
//	printk(__FILE__ ":%s: 4 mac addr = "MAC_FMT"\n", __func__, MAC_ARG(padapter->mac_addr));	// --- AP ---
#ifdef CONFIG_P2P
