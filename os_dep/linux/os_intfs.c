...
int rtw_os_ndev_register(_adapter *adapter, char *name)
{
	...
	ndev->addr_assign_type = NET_ADDR_PERM;								// --- AP ---
//	printk(__FILE__ ":%s: mac addr = "MAC_FMT"\n", __func__, MAC_ARG(ndev->dev_addr));		// --- AP ---

	/* Tell the network stack we exist */
	/* --- AP --- : "register_netdev" is in /net/core/dev.c */
...
//			printk(__FILE__ ":%s: mac addr = "MAC_FMT"\n", __func__, MAC_ARG(adapter->mac_addr));		// --- AP ---
